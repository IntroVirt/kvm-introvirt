...
touch $DIR/jammy/available-headers.txt
...
docker run -v $DIR:/opt --rm -it ubuntu:22.04 /bin/bash -c "apt-get update -y -qq && apt-cache search --names-only '^linux-headers-.*-generic' | sed -e 's/linux-headers-\(.*\)-generic .*/\1/' | sort -V > /opt/jammy/available-headers.txt"
