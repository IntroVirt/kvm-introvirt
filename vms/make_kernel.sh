...
# Copyright 2022 Assured Information Security, Inc.
...
make_initramfs() {
    ...
    rm -f sda && $sudo mknod sda b 8 0
    rm -f console && $sudo mknod console c 5 1
    rm -f ttyS0 && $sudo mknod ttyS0 c 4 64
    rm -f kvm && $sudo mknod kvm c 10 232

    make_init "$halt" "$autostart"
